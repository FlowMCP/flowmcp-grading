...
description: Consumes the JSON response from about-namespace-start-grade. Reads improvementHints[], decides whether to start a next iteration run (default maxIterations=3) or to write the final grading file to grading-data/namespace/<ns>/gradings/<hash>--<ts>--<basePersona>--<lens>.json (gitignored). On re-iteration it builds the re-invocation call to about-namespace-start-grade with iteration+1 and previousGradingPath. The decision matrix follows the grading spec. Persona application for the about-namespace area is mandatory (persona required).
...
Parameters (handed off from the `about-namespace-start-grade` skill):

| Parameter | Required | Format | Example |
|-----------|----------|--------|---------|
| `responseJson` | yes | Strict JSON per `about-namespace.schema.json` | `{ area: "about-namespace", iteration: 1, gradings: [...], improvementHints: [...] }` |
...
4a. **If re-iterating:**
    ...
    - Re-invoke `about-namespace-start-grade` with `iteration + 1`, `previousGradingPath`, and `personaSlug` unchanged.
    - The skill ends.
4b. **If finalizing:**
...
**Re-invocation:** Hand off to `about-namespace-start-grade` with:
...
With `<personaSlug> = <basePersona>--<lens>` (about-namespace requires a persona).
...
const { filename } = Grading.formatGradingFilename( {
    hash: schemaHash,
    ts: isoTs,
    persona: personaSlug          // '<basePersona>--<lens>' (about-namespace requires a persona)
} )
