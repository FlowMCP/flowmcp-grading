---
name: about-namespace-apply-improvement
description: Konsumiert die JSON-Response von about-namespace-start-grade. Liest improvementHints[], entscheidet ob ein naechster Iterations-Lauf startet (Default maxIterations=3) oder ob die finale Grading-Datei nach grading-data/namespace/<ns>/gradings/<hash>--<ts>--<basePersona>--<lens>.json (gitignored, Kap 4.6) geschrieben wird. Erzeugt bei Re-Iteration den Re-Invocation-Aufruf von about-namespace-start-grade mit iteration+1 + previousGradingPath. Decision-Matrix gemaess Memo 082 Kap 12. Persona-Anwendung Bereich 5 = MIT Persona Pflicht (Kap 7.4, Spec 11 §4).
...
Parameter (vom `about-namespace-start-grade`-Skill via Hand-off uebergeben):

| Parameter | Pflicht | Format | Beispiel |
|-----------|---------|--------|----------|
| `responseJson` | ja | Strict-JSON gemaess `about-namespace.schema.json` | `{ area: "about-namespace", iteration: 1, gradings: [...], improvementHints: [...] }` |
...
4a. **Wenn Re-Iteration:**
    ...
    - Re-invoke `about-namespace-start-grade` mit `iteration + 1`, `previousGradingPath`, `personaSlug` unveraendert.
    - Skill endet.
...
**Re-Invocation:** Hand-off an `about-namespace-start-grade` mit:
...
Mit `<personaSlug> = <basePersona>--<lens>` (Bereich 5 Persona-Pflicht, Kap 7.4 + Kap 13).
...
const { filename } = Grading.formatGradingFilename( {
    hash: schemaHash,
    ts: isoTs,
    persona: personaSlug          // '<basePersona>--<lens>' (Bereich 5 Persona-Pflicht)
} )
