...
description: Starts a grading iteration for the selection-skills-L3 area — a persona-based evaluation of the L3 skills of a selection. Loads the L3 template, pre-instructions, filtered questions (area=selection-skills, tier=L3), the output schema, and the persona (mandatory). Calls PromptBuilder.build, spawns a fresh sub-agent (read-only, empty context, strict JSON), and validates the response against prompts/output-schemas/selection-skills-L3.schema.json. Initializes the iteration counter and hands off to selection-skills-L3-apply-improvement. Note — selection-skills are more complex, so a small practical test run verifies the token budget.
...
|-----------|----------|--------|---------|
| `selectionPath` | yes | Absolute path to the selection (with selection lock + L3 skill file) | `/.../grading-data/selection/crypto-mini/` |
...
5. **Filter Questions** — Read `prompts/generated/questions.json`, filter `area == "selection-skills" && tier == "L3"`.
...
Strict JSON per `prompts/output-schemas/selection-skills-L3.schema.json` with required fields:

- `area: "selection-skills"` (literal match) + `tier: "L3"`
- `iteration`: Integer
- `personaSlug`: `<basePersona>--<lens>`
- `gradings[]`: Persona-specific L3 skill evaluations
...
After successful validation, hand off to `selection-skills-L3-apply-improvement` with:
...
1. **Parse JSON-Response** of the evaluator skill strictly against
   `prompts/output-schemas/selection-skills-L3.schema.json`. On parse error OR
...
- The evaluator skill (`selection-skills-L3-evaluate`), which is orchestrated here
