...
description: Konsumiert die JSON-Response von selection-skills-L3-start-grade. Liest improvementHints[], entscheidet ob ein naechster Iterations-Lauf startet (Default maxIterations=3) oder ob die finale Grading-Datei nach grading-data/selection/<sel>/gradings/<hash>--<ts>--<basePersona>--<lens>--L3.json (gitignored, Kap 4.6) geschrieben wird. Erzeugt bei Re-Iteration den Re-Invocation-Aufruf von selection-skills-L3-start-grade mit iteration+1 + previousGradingPath. Decision-Matrix gemaess Memo 082 Kap 12. Persona-Anwendung Bereich 7c = Pflicht (Kap 7.4, Spec 13 §4.2). User-Caveat REV-04 — Selection-Skill-Komplexitaet (Mini-Praxis-Test P6 verifiziert Token-Budget).
...
Parameter (vom `selection-skills-L3-start-grade`-Skill via Hand-off uebergeben):

| Parameter | Pflicht | Format | Beispiel |
|-----------|---------|--------|----------|
| `responseJson` | ja | Strict-JSON gemaess `selection-skills-L3.schema.json` | `{ area: "selection-skills", tier: "L3", iteration: 1, gradings: [...], improvementHints: [...] }` |
...
1. **Validate Input** — `responseJson` ist valid (`tier == "L3"`). `iteration >= 1`. `selectionPath` existiert. `personaSlug` matcht Pattern `<basePersona>--<lens>`.
...
4a. **Wenn Re-Iteration:**
    - Optional: Zwischen-State nach `grading-data/_tmp/<hash>--iteration-<n>--L3.json` (gitignored).
    - Re-invoke `selection-skills-L3-start-grade` mit `iteration + 1`, `previousGradingPath`, `personaSlug` unveraendert.
    - Skill endet.
4b. **Wenn Finalisieren:**
    - Berechne `<schemaHash>` (8-Zeichen sha256-Truncate, Spec 08 §5).
    - Berechne `<ISO-ts>` (`2026-MM-DDTHH-MM-SSZ`).
    - Berechne Zielpfad: `grading-data/selection/<selectionId>/gradings/<schemaHash>--<ISO-ts>--<personaSlug>--L3.json`.
    - `mkdir -p` Zielordner.
...
**Re-Invocation:** Hand-off an `selection-skills-L3-start-grade` mit:
...
Mit `<personaSlug> = <basePersona>--<lens>` (Bereich 7c Persona-Pflicht, Spec 13 §4.2 + Kap 13).
...
Filename-Bildung darf nur via `Grading.formatGradingFilename({ hash, ts, persona })` aus `src/Grading.mjs` laufen — **kein** String-Concat im Save-Step. Der `--L3`-Tier-Suffix wird im Aufrufer nach dem Helper-Ergebnis angehaengt.
...
const { filename } = Grading.formatGradingFilename( {
    hash: schemaHash,
    ts: isoTs,
    persona: personaSlug          // '<basePersona>--<lens>' (Bereich 7c Persona-Pflicht)
} )
