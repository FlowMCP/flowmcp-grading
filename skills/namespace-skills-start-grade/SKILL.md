...
description: Starts a grading iteration for the namespace-skills area — persona-based assessment of a namespace's skills. Loads the template, pre-instructions, filtered questions (area=namespace-skills), the output schema, and the persona (recommended default). Calls PromptBuilder.build, spawns a fresh sub-agent (read-only, empty context, strict JSON), and validates the response against prompts/output-schemas/namespace-skills.schema.json. Initializes the iteration counter and hands off to namespace-skills-apply-improvement. The namespace-skills area uses a persona (personaRequired: true) as the recommended default.
...
**Persona application:** `personaRequired: true` — the recommended default.

## Procedure

1. **Validate inputs** — `namespacePath` exists. `personaSlug` is set. On error: `{ "blocker": "personaSlug", "reason": "missing — personaRequired: true (recommended default)" }`.
...
5. **Filter questions** — read `prompts/generated/questions.json`, filter `area == "namespace-skills"`.
6. **Load persona (default)** — split `personaSlug` into `<basePersona>--<lens>`. Read the base persona from `repos/flowmcp-spec/personas/<basePersona>.md`. Read the lens helper from `flowmcp-grading/grading-data/personas/<lens>-<YYYY>.md`.
...
10. **Validate response** — parse JSON. Validate against `namespace-skills.schema.json`. On schema failure: `{ "blocker": "schema-validation", "reason": "<details>" }`.
11. **Hand-off** — call `namespace-skills-apply-improvement` with the JSON + `iteration` + `namespacePath` + `personaSlug`.

## Output

Strict JSON per `prompts/output-schemas/namespace-skills.schema.json` with required fields:

- `area: "namespace-skills"` (literal match)
- `iteration`: integer
- `personaSlug`: `<basePersona>--<lens>`
- `gradings[]`: persona-specific namespace-skill assessments
...
After successful validation, hand off to `namespace-skills-apply-improvement` with:
...
1. **Parse the JSON response** of the evaluator skill strictly against
   `prompts/output-schemas/namespace-skills.schema.json`. On parse error OR
...
- Evaluator skill (`namespace-skills-evaluate`), orchestrated here
