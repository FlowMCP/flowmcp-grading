...
description: Starts a grading iteration for the tools-aggregate-schema area — a neutral evaluation of the coherence across all routes of a schema. Loads the template, pre-instructions, filtered questions (area=tools-aggregate-schema), and the output schema. Calls PromptBuilder.build, spawns a fresh sub-agent (read-only, empty context, strict JSON), and validates the response against prompts/output-schemas/tools-aggregate-schema.schema.json. Initializes the iteration counter and hands off to tools-aggregate-schema-apply-improvement. Per the grading spec, the tools-aggregate-schema area is graded neutrally (personaRequired: false).
...
|-----------|----------|--------|---------|
| `schemaPath` | yes | Absolute path to the schema (all routes are aggregated) | `/.../schemas-private/v3/etherscan/getContractEthereum.mjs` |
| `personaSlug` | no (tools-aggregate-schema is neutral) | `"neutral"` (literal) | `neutral` |
| `iteration` | no (default 1) | Integer 1..N | `1` |
| `previousGradingPath` | only from iteration 2 onward | Absolute path | `/.../grading-data/single/.../gradings/abc--ts--neutral.json` |

**Persona handling:** `personaRequired: false`. The coherence evaluation is structural, hence neutral.

## Process

1. **Validate Inputs** — `schemaPath` exists. On error: `{ "blocker": "schemaPath", "reason": "not found" }`.
2. **Load Template** — Read `prompts/templates/tools-aggregate-schema.md`.
3. **Load Pre-Instructions** — Read `prompts/pre-instructions/tools-aggregate-schema.md` (files-to-read = all routes of the schema).
4. **Load Output-Schema** — Read `prompts/output-schemas/tools-aggregate-schema.schema.json`.
5. **Filter Questions** — Read `prompts/generated/questions.json`, filter `area == "tools-aggregate-schema"`.
...
8. **Build Prompt** — Call `PromptBuilder.build({ template, preInstructions, outputSchema, questions, persona: null, previousHints, schemaPath, iteration })`.
...
10. **Validate Response** — Parse JSON. Validate against `tools-aggregate-schema.schema.json`. On schema failure: `{ "blocker": "schema-validation", "reason": "<details>" }`.
11. **Hand-off** — Call `tools-aggregate-schema-apply-improvement` with the JSON + `iteration` + `schemaPath` + `personaSlug="neutral"`.

## Output

Strict JSON per `prompts/output-schemas/tools-aggregate-schema.schema.json` with required fields:

- `area: "tools-aggregate-schema"` (literal match)
- `iteration`: Integer
- `personaSlug: "neutral"`
- `gradings[]`: Question answers aggregated across all routes
...
After successful validation, hand off to `tools-aggregate-schema-apply-improvement` with:

- `responseJson` — the validated JSON from step 10
- `iteration` — current iteration (default start = 1)
- `schemaPath` — unchanged
- `personaSlug` — `"neutral"` (the tools-aggregate-schema area is neutral)
...
1. **Parse JSON-Response** of the evaluator skill strictly against
   `prompts/output-schemas/tools-aggregate-schema.schema.json`. On parse error OR
...
- The evaluator skill (`tools-aggregate-schema-evaluate`), which is orchestrated here
